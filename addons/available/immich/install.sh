...
if [[ -f "${SCRIPT_DIR}/../../src/hardware-detect.sh" ]]; then
    ...
    if [[ $available_memory_gb -ge 16 ]]; then
        # High-end system
        SERVER_MEMORY_LIMIT=${SERVER_MEMORY_LIMIT:-"4g"}
        SERVER_CPU_LIMIT=${SERVER_CPU_LIMIT:-"2.0"}
        MICROSERVICES_MEMORY_LIMIT=${MICROSERVICES_MEMORY_LIMIT:-"2g"}
        DB_MEMORY_LIMIT=${DB_MEMORY_LIMIT:-"2g"}
        DB_CPU_LIMIT=${DB_CPU_LIMIT:-"1.5"}
        MAX_FILE_SIZE=${MAX_FILE_SIZE:-"500"}
        MAX_NUMBER_OF_FILES=${MAX_NUMBER_OF_FILES:-"50000"}
        ENABLE_MACHINE_LEARNING=${ENABLE_MACHINE_LEARNING:-"true"}
        ...
        MICROSERVICES_MEMORY_LIMIT=${MICROSERVICES_MEMORY_LIMIT:-"1g"}
        DB_MEMORY_LIMIT=${DB_MEMORY_LIMIT:-"1g"}
        DB_CPU_LIMIT=${DB_CPU_LIMIT:-"1.0"}
        MAX_FILE_SIZE=${MAX_FILE_SIZE:-"200"}
        MAX_NUMBER_OF_FILES=${MAX_NUMBER_OF_FILES:-"10000"}
        ENABLE_MACHINE_LEARNING=${ENABLE_MACHINE_LEARNING:-"true"}
        ...
        MICROSERVICES_MEMORY_LIMIT=${MICROSERVICES_MEMORY_LIMIT:-"512m"}
        DB_MEMORY_LIMIT=${DB_MEMORY_LIMIT:-"512m"}
        DB_CPU_LIMIT=${DB_CPU_LIMIT:-"0.5"}
        MAX_FILE_SIZE=${MAX_FILE_SIZE:-"100"}
        MAX_NUMBER_OF_FILES=${MAX_NUMBER_OF_FILES:-"5000"}
...
EOF
        ...
        mkfs.ext4 -F -L "immich-storage" "$STORAGE_DEVICE"
    ...
    # Opret Immich directory struktur
    log "Opretter Immich directory struktur..."
    mkdir -p "$LIBRARY_DIR" "$UPLOAD_DIR" "$THUMBNAIL_DIR" "$PROFILE_DIR"
    
    # Sæt permissions
    chown -R "$SERVICE_USER:$SERVICE_USER" "$STORAGE_MOUNT"
    chmod 755 "$STORAGE_MOUNT"
    chmod 755 "$LIBRARY_DIR" "$UPLOAD_DIR" "$THUMBNAIL_DIR" "$PROFILE_DIR"
...
install_containers() {
    log "Installerer Immich containers..."
    
    # Pull images
    local images=(
        "ghcr.io/immich-app/immich-server:${IMAGE_VERSION}"
        "ghcr.io/immich-app/immich-machine-learning:${IMAGE_VERSION}"
        "redis:7-alpine"
        "postgres:14-alpine"
    )
...
services:
  immich-server:
    image: ghcr.io/immich-app/immich-server:${IMAGE_VERSION}
    container_name: immich_server
    command: ['start.sh', 'immich']
    volumes:
      - ${UPLOAD_DIR}:/usr/src/app/upload
      - ${LIBRARY_DIR}:/usr/src/app/library
      - ${THUMBNAIL_DIR}:/usr/src/app/thumbnails
      - ${PROFILE_DIR}:/usr/src/app/profile
    environment:
      - NODE_ENV=production
  ...
  immich-machine-learning:
    image: ghcr.io/immich-app/immich-machine-learning:${IMAGE_VERSION}
...
[Unit]
Description=Immich Photo Service (Humethix)
...
start_service() {
    log "Starter Immich service..."
    systemctl start "$ADDON_NAME"
    
    # Vent på at containers starter
    log "Venter på at Immich starter (dette kan tage et par minutter)..."
    sleep 30
    ...
    while [[ $retries -gt 0 ]]; do
        if curl -f -s "http://127.0.0.1:${WEB_PORT}" &>/dev/null; then
            log "Immich web interface er klar!"
            break
        fi
        sleep 2
        ((retries--))
    ...
    if [[ $retries -eq 0 ]]; then
        warn "Immich web interface svarer ikke endnu - tjek logs med: journalctl -u $ADDON_NAME -f"
    else
...
show_storage_info() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  IMMICH STORAGE INFORMATION"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
main() {
    ...
    log "Immich tilgængelig på: http://127.0.0.1:${WEB_PORT}"
    if [[ "${EXPOSE_EXTERNAL:-false}" == "true" ]]; then
    ...
    log "Upload dine billeder til: $LIBRARY_DIR"
}
